...
# 目标检测函数
def my_detection(img_path,name):
    base_scores=0
    ...
    # 只选取得分大于base_scores的检测结果
    obj_index = np.argwhere(scores > base_scores).squeeze(axis=1).tolist()
    ...
    #origin_img.show()
    # 将检测结果保存
    origin_img.save(f"./result/{model_name}{base_scores}+{name}")
...
if __name__ == '__main__':
    ...
    for path in filenames:
        file_Path=f"./img/{path}"
        my_detection(file_Path,path)  # 对给定图像进行目标检测